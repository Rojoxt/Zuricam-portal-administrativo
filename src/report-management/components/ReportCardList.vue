...
<template>
    ...
    >
      ...
      <template v-slot:item.actions="{ item}">
        <v-btn size="small" class="ma-1 " color="primary" icon="mdi-pencil"
               @click="editReport( {item : item}) "></v-btn>
        <v-btn size="small" class="ma-1" color="error" icon="mdi-delete"
               @click="confirmDeleteReport({item : item})"></v-btn>
      </template>
  ...
  <v-dialog v-model="reportDialog" max-width="500">
    <v-card prepend-icon="mdi-account" title="Perfil de camara">
      ...
      <v-card-text>
        <v-text-field density="compact" label="Dirección*" variant="outlined" color="secondary" required
                      v-model.trim="report.address"></v-text-field>
        <v-text-field density="compact" v-model.trim="report.incident" label="Lugar*" variant="outlined"
                      color="secondary" required></v-text-field>
        <v-text-field density="compact" label="Enlace*" variant="outlined" color="secondary" required
                      v-model.trim="report.trackingLink"></v-text-field>
        <v-text-field density="compact" v-model.trim="report.image" label="Imagen*" variant="outlined"
                      color="secondary" required></v-text-field>
        <v-combobox
          density="compact"
          variant="outlined"
      ...
      <v-card-actions>
        ...
        <v-btn
          ...
          text="Save"
          variant="tonal"
          @click="saveReport({report : report})"
        ></v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
